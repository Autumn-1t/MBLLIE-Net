...
class MBPNModel(BaseModel, torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['MP','Color','Pix','Adv','T']
...
# GRU状态（hidden state）初始化
    def get_h(self,cdip,sdip):#cdip代表隐状态的深度（通道数），sdip代表隐状态的空间降采样率
        b, c, w, h = self.real_LI.shape#b是批次大小，c是通道数，w和h分别是宽度和高度
        return torch.ones((b, cdip, w//sdip, h//sdip))

    def init(self):
        self.t_h = {}#gru last output，这是一个字典，包含了GRU每一层的隐状态
        #调用函数初始化四个不同深度和空间降采样率的隐状态，分别存储在字典中
    ...
    def forward(self):
        ...
        i = 0
        while i <= 2:
            self.boost_HI, self.t_h, self.boost_HI2, self.boost_HI4 = \
                                        self.netG(torch.cat((self.real_LI, self.boost_HIS[-1], self.boost_HIS[-1]), 1), self.t_h)
    ...
    def backward_G(self):
        ...
        self.loss_T =self.loss_MP+self.loss_Adv+self.loss_Color+self.loss_Pix
