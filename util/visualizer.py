...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        if self.use_traini:
            self.traini_dir = os.path.join(opt.checkpoints_dir, opt.name, 'train')
    ...
    #     with open(self.log_name, "a") as log_file:
    #         print(type(total_time), total_time)  # This will help verify the type and value of total_time
    #         log_file.write('Total training time: {:.1f} hours \n' % total_time/60/60)
    def log_total_time(self, total_time):
        with open(self.log_name, "a") as log_file:
            hours = total_time / 3600  # This ensures the calculation is done before formatting
            log_file.write('Total training time: {:.1f} hours \n'.format(hours))
